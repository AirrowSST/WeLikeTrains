...
- User approved shelter-aware route detail on 2026-09-19: walking legs retain covered, exposed and unknown distances and matching map geometry. Preferences distinguish **Prefer mapped shelter** from **Require mapped shelter**; required mode must not silently accept exposed or materially unknown access. Weather may temporarily prefer shelter without changing saved preferences. Exposed and unknown sections remain visibly distinct on normal and active-journey maps, and incomplete coverage must not be called fully sheltered.

- User approved Google-backed island-wide online address discovery on 2026-09-19, then rejected the visually inconsistent Google-rendered Places UI Kit controls. Configured normal mode now uses Wayce's standard input and dropdown backed by Google Place Autocomplete Data API, with Singapore restriction, per-edit session tokens and visible required Google Maps attribution. Google supplies suggestions and the selected place details only; Wayce still calculates routes on its local OSM/DataMall graph and displays them on OneMap/OSM. Preserve indexed local search when the provider is unconfigured, unavailable or offline, disclose that typed online searches go to Google, and do not hide Google attribution or add Google routing/basemap behavior without a new decision.
...
- User decided on 2026-09-19 that heavy rain alone must not make a walking journey unavailable or produce **Wait for safer conditions**. Prefer routes with less mapped exposure, retain a realistic rain delay, and advise an umbrella plus care on slippery paths and in reduced visibility. Explicit floods, closures, unavailable segments and access restrictions remain blocking; heavy-rain cycling exclusion remains unchanged.
