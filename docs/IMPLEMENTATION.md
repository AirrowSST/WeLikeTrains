...
- Browser coverage stubs OneMap tiles so tests remain deterministic and no live provider is required. It separately checks the detailed online state and the bundled offline fallback alongside local search/routing, the resizable one-column layout, chat route cards, foreground/simulated location, 320px no-overflow, axe and offline reload.
- OneMap restoration verification: the official service returned HTTP 200 PNG tiles for representative west, north, east, southern-island and central coordinates; TypeScript, production build, all 25 unit tests, the primary mobile journey browser test and the offline reload browser test pass. Local visual inspection shows the detailed coloured basemap, required attribution and route overlays together. This is not a physical-phone sign-off.
