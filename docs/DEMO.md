...
8. Switch **Data & sources → Live feeds**. Show real advisory messages, feed freshness and crowd readings. Explain that OneMap supplies only the visual basemap, configured Google Places UI Kit supplies an online selected coordinate, and route computation remains on the bundled OSM/DataMall data. A quiet live disruption feed is normal; do not relabel the synthetic event as live.
