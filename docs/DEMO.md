...
1. Open the HTTPS URL in a **real phone browser**. Confirm the mobile-only bottom navigation and one-column UI starts as Guest. Open **Account**, enable **Developer mode**, choose **Open demo presets**, then load Rachel.
...
6. Open the companion and read the consent text. Tap the microphone, dictate “Why this route?”, review the transcript before sending, and show the response provider label. Explain that browser speech recognition may use the browser vendor's online service and that Wayce receives only the sent draft. Ask “I avoid crowds and need sheltered walks”, review the proposal and apply it. Read a response aloud.
7. Tap **Use simulated location** and confirm the origin, status and amber map marker all say the position is simulated. Start the journey and manually advance one step; the simulated marker follows the confirmed step. Explain that Live mode requests a one-shot browser location when it opens, continuous foreground tracking stops when the journey closes, and steps never auto-complete. With the app previously loaded, turn on airplane mode and reload. Confirm the map/steps remain and the offline/stale warning is prominent. Turn airplane mode off afterwards.
8. Switch **Data & sources → Live feeds**. Show real advisory messages, feed freshness and crowd readings. Explain that OneMap supplies only the visual basemap, configured Google Place Autocomplete supplies attributed suggestions and a selected coordinate inside Wayce's UI, and route computation remains on the bundled OSM/DataMall data. A quiet live disruption feed is normal; do not relabel the synthetic event as live.
...
- Location: on a compatible phone over HTTPS, open Live mode, grant the automatically requested location permission yourself, and confirm the blue device marker and accuracy label. Deny once and confirm the visible retry/manual-origin fallback. Start/stop foreground tracking in the journey dialog and confirm closing it stops tracking. Do not grant permission on behalf of a judge or claim a physical-phone result until performed.
...
- [ ] Microphone permission, transcription, stop/retry and manual-send behavior work on the target phone browser; audio is not claimed to be processed by Wayce.
