...
Not implemented as current product claims: historical disruption-model training, passenger-volume ML, every suggested DataMall endpoint, authoritative covered-linkway/exit layers, background GPS, voice recognition, or a native application. These are not silently simulated.
